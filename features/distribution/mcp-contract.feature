...
Feature: The MCP is a thin plug over the same core
  ...
  Scenario: Writing through the plug is writing through the product
    ...
    Then the response is not an error
    When I run "roca exec 'SELECT (SELECT COUNT(*) FROM main.memories WHERE supersedes IS NULL) + (SELECT COUNT(*) FROM plugin_roca_ops.memories WHERE supersedes IS NULL) AS n' --json"
