...
func TestStatusCommandExitsWhenInspectDeadlineFires(t *testing.T) {
	previous := terminateProcess
	called := false
	terminateProcess = func(status int) {
		called = true
		if status != 0 {
			t.Fatalf("status deadline exit = %d, want 0 after printing facts", status)
		}
	}
	t.Cleanup(func() { terminateProcess = previous })
	abandonStatusIfDeadlineExceeded(context.DeadlineExceeded)
	if !called {
		t.Fatal("status deadline left the process waiting for leftover inspects")
	}
	called = false
	abandonStatusIfDeadlineExceeded(nil)
	if called {
		t.Fatal("finished status abandoned the process")
	}
}

func TestWorkerTerminatesProcessOnNativeStall(t *testing.T) {
	previous := terminateProcess
	var code int
	called := false
	terminateProcess = func(status int) {
	...
	t.Cleanup(func() { terminateProcess = previous })
	abandonWorkerIfStuck(vector.Completion{ExitStatus: 1, Error: "semantic search stalled while preparing embeddings"})
	if !called || code != 1 {
		t.Fatalf("terminateProcess called=%t code=%d, want exit 1 on native stall", called, code)
	}
	called = false
	abandonWorkerIfStuck(vector.Completion{ExitStatus: 1, Error: "download the embedding model"})
	if called {
		t.Fatal("non-stall worker error abandoned the process")
	}
	abandonWorkerIfStuck(vector.Completion{})
	if called {
		t.Fatal("successful worker abandoned the process")
	}
}
