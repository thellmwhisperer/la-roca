...
func registerDistributionSkillSteps(ctx *godog.ScenarioContext, w *distributionWorld) {
	...
	ctx.Then(`^shared hook configurations are refused unchanged and separate session scripts are installed$`,
		w.hookInstallationPreservesExistingFiles)
...
// Shared configurations must be refused unchanged; separate scripts can be created.
var distributionHookFixtures = map[string]struct{ path, body string }{
	"claude": {
...
func (w *distributionWorld) installSessionHooksEverywhere() error {
	for _, runtime := range distributionHookRuntimes() {
		run := w.run("hooks", "install", runtime, "--pills", "--handoff")
		if runtime == "pi" || runtime == "opencode" {
			if run.code != 0 {
				return fmt.Errorf("hooks install %s failed: %s%s", runtime, run.stdout, run.stderr)
			}
		} else if run.code != 1 || !strings.Contains(run.stderr, "atomic conditional replacement is unsupported") {
			return fmt.Errorf("hooks install %s: want conditional refusal, got code %d: %s", runtime, run.code, run.stderr)
		}
	}
	return nil
}

func (w *distributionWorld) hookInstallationPreservesExistingFiles() error {
	for _, runtime := range distributionHookRuntimes() {
		fixture := distributionHookFixtures[runtime]
		path := filepath.Join(w.home, fixture.path)
		body, err := os.ReadFile(path)
		if err != nil || string(body) != fixture.body {
			return fmt.Errorf("installation changed %s's existing hook: %v", runtime, err)
		}
		if runtime == "pi" || runtime == "opencode" {
			...
			if err != nil || string(backup) != fixture.body {
				return fmt.Errorf("%s hook backup does not preserve the original bytes: %v", runtime, err)
			}
		}
	}
	return nil
...
func (w *distributionWorld) withdrawalLeavesOnlyTheForeignHook() error {
	for _, runtime := range distributionHookRuntimes() {
		...
		if err != nil || string(body) != fixture.body {
			return fmt.Errorf("withdrawal changed %s's existing hook: %v", runtime, err)
		}
		if runtime == "pi" || runtime == "opencode" {
