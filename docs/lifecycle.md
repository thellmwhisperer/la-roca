...
database, configuration, indexes, logs, generated prompt, backups, skills, the
installed [vector search](vector.md) store, and integration recovery copies,
plus the credential files and model catalogue cache that older releases left
behind. The recovery copies a refresh left beside a managed artifact belong to
the same family: a regular uninstall names them as kept, and a purge takes them
with the rest, so the directory holding them can be taken back too.
