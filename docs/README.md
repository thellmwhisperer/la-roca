...
2. [The MCP plug](mcp.md): the stdio server, its five tools, the TOON answer
   contract, and the supported integration targets.
3. [Install, update, and uninstall](lifecycle.md): the binary's life cycle,
   verification on update, and consent on uninstall.
4. [Operations](operations.md): operational logs, redaction, retention, and
   the read-only boundary.
5. [Architecture](architecture.md): the four internal domains and the import
