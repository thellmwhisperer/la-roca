...
Owner decision: accept D7 against the transaction limit and no time regression, and withdraw the 3x gate. D7 is `removed` with its existing forbid and the rule of 64-chunk batches with progress and activity persisted per batch. No native-throughput investigation or other wave work is authorized. All measurements remain unchanged; this evidence accompanies the scoped patch into no-mistakes validation.
