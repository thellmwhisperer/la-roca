...
func CatalogBody(databases []plugin.Database, notes []string) string {
	...
	b.WriteString("  exact SQL yourself. Prefer the roca skill and its `roca query` and\n")
	b.WriteString("  `roca explore`; this catalog is for authors of `roca exec` SELECTs.\n")
	b.WriteString("---\n\n")
