...
`), 0o600); err != nil {
	...
	rides, warnings, err = DiscoverOperatorRides(path, "")
	if err != nil || len(rides) != 1 || len(warnings) != 0 || rides[0].Name != "backup" {
		t.Fatalf("trusted discovery = rides=%+v warnings=%v err=%v", rides, warnings, err)
	}
	...
	if err == nil || len(rides) != 0 || len(warnings) != 0 ||
		!strings.Contains(err.Error(), "writable by group or others") {
		t.Fatalf("other-writable discovery = rides=%+v warnings=%v err=%v", rides, warnings, err)
	}
}
