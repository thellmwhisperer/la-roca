...
func (i Index) Ingest(ctx context.Context) (Delta, error) {
	...
	release, err := securefile.Lock(i.VectorPath + ".index.lock")
	if err != nil {
		return Delta{}, fmt.Errorf("lock vector index: %w", err)
	}
	defer release()

	core, err := openSQLite(i.CorePath, true)
	if err != nil {
		return Delta{}, fmt.Errorf("open core database: %w", err)
	}
	defer core.Close()
	...
	err = walkSources(ctx, core, func(source sourceRow) error {
		report.Sources++
...
func (i Index) Query(ctx context.Context, text string, k int) ([]Result, error) {
	...
	for _, candidate := range candidates {
		...
		body, err := resolveSource(ctx, core, candidate.kind, candidate.where)
		if err != nil {
			return nil, err
		}
		if body == "" {
...
func (i Index) validate() error {
	if i.CorePath == "" || i.VectorPath == "" {
		return fmt.Errorf("core and vector database paths are required")
	}
	if i.Model == "" {
...
func resolveSource(ctx context.Context, db *sql.DB, kind string, where locator) (string, error) {
	var row *sql.Row
	switch kind {
	case "sessions":
	...
	case "memories":
		...
		default:
			return resolveDirectMemory(ctx, db, where)
		}
	default:
		return "", fmt.Errorf("unknown vector source %q", kind)
	}
	var text string
	if err := row.Scan(&text); err == sql.ErrNoRows {
		return "", nil
	} else if err != nil {
